...
@Component({
      ...
      <div class="remotes-grid">
        @for (remote of remotes; track remote.name) {
          <a [routerLink]="remote.route" class="remote-card">
            ...
            <div class="remote-framework" [style.color]="remote.color">{{ remote.framework }}</div>
            <div class="remote-meta">Port {{ remote.port }} · Native Federation</div>
          </a>
        }
      </div>
  ...
  styles: [`
    ...
    .remote-name { font-weight: 600; font-size: 15px; }
    ...
    .remote-status.loading { background: rgba(249, 115, 22, 0.15); color: var(--accent-orange); }
    .remote-framework { font-size: 24px; font-weight: 700; margin: 8px 0; }
    .remote-meta { font-size: 12px; color: var(--text-secondary); }
  `],
})
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.intervalId = setInterval(() => {
      ...
      if (seconds < 60) this.uptime = `${seconds}s`;
